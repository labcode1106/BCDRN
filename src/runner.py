...
class Runner(object):
    ...
    def detect(self):
        ...
        for relativePath, dataSet in self.corpus.dataFiles.iteritems():
            ...
            if self.useDataReduction and self.useOr:
                labels = util.convertLabelsBasedOnDataReduction(labels, self.combinedNum)
            # windows = self.corpusLabel.windows[relativePath]
            # if self.useDataReduction and self.useOr:
            #     windows = util.convertWindowsForDataReduction(windows, dataSet, self.combinedNum)
            ...
            detector = Detector(self.useDataReduction, self.useOr, self.useConv, self.useAverage, self.combinedNum,
                                self.convWidth, self.poolingWidth)
    ...
    def normalize(self):
        ...
        if self.useDataReduction:
            if self.useOr:
                combinedNumResult = self.combinedNum
            if self.useConv:
        ...
        dt = [str(self.useDataReduction), combinedNumResult, convWidthResult, poolingWidthResult, str(self.useAverage)] + finalScores
